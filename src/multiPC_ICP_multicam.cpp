...
{
	// Initialize ROS
  ros::init (argc, argv, "multiPC_ICP_multicam");
  ros::NodeHandle nh;
  ...
  listener.waitForTransform("/cell_link","camera1_rgb_optical_frame",ros::Time::now(),ros::Duration(1.0));
  listener2.waitForTransform("/cell_link","camera2_rgb_optical_frame",ros::Time::now(),ros::Duration(1.0));
  listener3.waitForTransform("/cell_link","camera3_rgb_optical_frame",ros::Time::now(),ros::Duration(1.0));
  listener4.waitForTransform("/world","/cell_link",ros::Time::now(),ros::Duration(1.0));
  ...
  listener.lookupTransform("cell_link","camera1_rgb_optical_frame", ros::Time::now(), transform);
  listener2.lookupTransform("cell_link","camera2_rgb_optical_frame", ros::Time::now(), transform2);
  listener3.lookupTransform("cell_link","camera3_rgb_optical_frame", ros::Time::now(), transform3);
  listener4.lookupTransform("/world","/cell_link", ros::Time::now(), transform4);
  }
  ...
  while(ros::ok())
  {
	  ...
	  if (cam_now==1) {
		  sub = nh.subscribe("/camera1/depth_registered/points",1,cloud_cb);
		  cam_now=0;
	  }
	  else if(cam_now==2){
  ...
  pcl::fromROSMsg(cloud_in1, *cloud1_init);
  pcl::fromROSMsg(cloud_in2, *cloud2_init);
  pcl::fromROSMsg(cloud_in3, *cloud3_init);

  pcl::PassThrough<PointType> pass_cloud1;
  pass_cloud1.setInputCloud (cloud1_init);
  pass_cloud1.setFilterFieldName ("z");
  pass_cloud1.setFilterLimits (0.5, 1.8);
  pass_cloud1.filter (*cloud1_init);

  pcl::PassThrough<PointType> pass_cloud2;
  pass_cloud2.setInputCloud (cloud2_init);
  pass_cloud2.setFilterFieldName ("z");
  pass_cloud2.setFilterLimits (0.5, 1.8);
  pass_cloud2.filter (*cloud2_init);

  pcl::PassThrough<PointType> pass_cloud3;
  pass_cloud3.setInputCloud (cloud3_init);
  pass_cloud3.setFilterFieldName ("z");
  pass_cloud3.setFilterLimits (0.5, 1.8);
  pass_cloud3.filter (*cloud3_init);

  pcl::toROSMsg(*cloud1_init,cloud_inter1);
  cloud_inter1.header=cloud_in1.header;
  pcl::toROSMsg(*cloud2_init,cloud_inter2);
  cloud_inter2.header=cloud_in2.header;
  pcl::toROSMsg(*cloud3_init,cloud_inter3);
  cloud_inter3.header=cloud_in3.header;

  pcl_ros::transformAsMatrix (transform, eigen_transform);
//  std::cout<<eigen_transform<<std::endl;
  pcl_ros::transformPointCloud(eigen_transform,cloud_inter1,input1);
  input1.header.frame_id="/cell_link";
  pcl_ros::transformAsMatrix (transform2, eigen_transform2);
  //std::cout<<eigen_transform2<<std::endl;
  pcl_ros::transformPointCloud(eigen_transform2,cloud_inter2,input2);
  pcl_ros::transformAsMatrix (transform3, eigen_transform3);
  pcl_ros::transformPointCloud(eigen_transform3,cloud_inter3,input3);
  input3.header.frame_id="/cell_link";
 ...
 // std::cout<<cloud_filtered->size()<<std::endl;
  ...
  if((prev_fit-fit)>5e-6 && fit<2.7e-5){
	  do_tr.data=1;
  }
  else{
	  do_tr.data=0;
